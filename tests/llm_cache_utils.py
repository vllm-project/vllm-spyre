...
class SortKey(NamedTuple):
    ...
    def _get_tp_size(item) -> int:
        ...
        for key in TP_KEYS:
            if key in params:
                SortKey._assert_param(
                    isinstance(params[key], int), "tp size must be an int", item
                )
                return params[key]
    ...
    def _get_model(item) -> str:
        ...
        for key in MODEL_KEYS:
            if key in params:
                SortKey._assert_param(
                    isinstance(params[key], str), "model must be a string", item
                )
                return params[key]
    ...
    def _get_backend(item) -> str:
        if "backend" in item.callspec.params:
            ...
            SortKey._assert_param(
                isinstance(backend, str), "backend must be a string.", item
            )
            return backend
    ...
    def _get_num_blocks(item) -> int:
        if "available_blocks" in item.callspec.params:
            ...
            SortKey._assert_param(
                isinstance(blocks, int), "available_blocks must be an int.", item
            )
            return blocks
        ...
        *,
        ...
        if seed is not None:
            if "--seed" in vllm_serve_args:
                raise ValueError(
                    "You have manually specified the seed " f"when `seed={seed}`."
                )
...
class RemoteOpenAIServerCache:
    ...
    def get_api_server(
        self, model: str, server_args: list[str], server_env: dict
    ) -> RemoteOpenAIServer:
        ...
        return self._cache.set(
            runtime_config,
            RemoteOpenAIServer(
                model=model, vllm_serve_args=server_args, env_dict=server_env
            ),
        )
...
class EngineCache:
    ...
    def get_engine(
        ...
        patch_environment(
            use_cb=True, warmup_shapes=None, backend=backend, monkeypatch=monkeypatch
        )
