...
def decode(batch_update_builder: BatchUpdateBuilder, lp: LogitsProcessor,
...
def test_mintokens_logits_processor():
    """
    This method tests the MinTokensLogitsProcessor class.
    ...
    lp = MinTokensLogitsProcessor(dummy_config, device, False)
...
def test_logitbias_logits_processor():
    ...
    dummy_config = DummyVllmConfig()
    lp = LogitBiasLogitsProcessor(dummy_config, device, False)
...
def test_minp_logits_processor():
    ...
    dummy_config = DummyVllmConfig()
    lp = MinPLogitsProcessor(dummy_config, device, False)
    ...
    assert torch.allclose(vllm_logits[1], out_logits[1])
