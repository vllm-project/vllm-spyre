...
from output_util import ISCLOSE_ABS_TOL, ISCLOSE_ABS_TOL_QUANTIZATION, compare_results, generate_hf_output
...
def generate_prompts(
    ...
    for i, (add_step, max_tokens, prompt_length) in enumerate(sorted_reqs_params):
        ...
        sampling_params = SamplingParams(max_tokens=max_tokens, temperature=0.0, logprobs=0, ignore_eos=True)
        request = create_random_request(
            request_id=i, num_tokens=prompt_length, sampling_params=sampling_params, model=model
        )
...
def check_scheduler_inference_steps(
    ...
    # ------
    if not (len(prompts_lengths) == len(seqs_max_tokens) and len(prompts_lengths) == len(steps_add_reqs)):
    ...
    if not (steps_add_reqs == sorted(steps_add_reqs) and steps_add_reqs[0] == 0):
        raise ValueError("The list of steps where requests are added should be increasing start with 0")
    ...
    collected_outputs = defaultdict(lambda: {"token_ids": [], "logprobs": [], "text": "", "tokens": []})

    prompts, requests = generate_prompts(model, steps_add_reqs, seqs_max_tokens, prompts_lengths)
    ...
    for step in range(checked_steps[-1]["step"] + 1):
        ...
        if checked_steps and step == checked_steps[0]["step"]:
            ...
            assert DISABLE_ASSERTS or (scheduler.tkv == step_ref["tkv"]), f"Step {step}, tkv: {scheduler.tkv}"
            assert DISABLE_ASSERTS or waiting == step_ref["waiting"], f"Step {step}, waiting: {waiting}"
            assert DISABLE_ASSERTS or running == step_ref["running"], f"Step {step}, running: {running}"
            ...
            n_blocks = engine_core.model_executor.driver_worker.worker.model_runner.n_blocks
            n_reserved_blocks = n_blocks - scheduler.n_free_blocks
            req_ids2blocks = engine_core.model_executor.driver_worker.worker.model_runner.req_ids2blocks
            req_ids2reserved_blocks = (
                engine_core.model_executor.driver_worker.worker.model_runner.req_ids2reserved_blocks
            )
            n_used_blocks = sum([len(blocks) for blocks in req_ids2blocks.values()])
            ...
            assert DISABLE_ASSERTS or len(req_ids2blocks) == len(req_ids2reserved_blocks)
            for req_id in req_ids2blocks:
                # current number of used blocks should be less than reserved
                assert DISABLE_ASSERTS or len(req_ids2blocks[req_id]) <= req_ids2reserved_blocks[req_id]
        ...
        for output in request_outputs:
            ...
            collected_outputs[output.request_id]["tokens"].append(tokenizer.decode(new_token_ids[0]))
