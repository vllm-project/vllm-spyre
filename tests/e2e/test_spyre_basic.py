...
@pytest.mark.parametrize("backend", [pytest.param("sendnn_decoder", marks=pytest.mark.spyre, id="sendnn_decoder")])
def test_output_sendnn_decoder(
    model: ModelInfo, warmup_shapes: DecodeWarmupShapes, backend: str, monkeypatch: pytest.MonkeyPatch, use_llm_cache
) -> None:
    """
...
def test_batch_handling(
    ...
    vllm_sampling_params = [
        SamplingParams(
            max_tokens=max_new_tokens[i], min_tokens=max_new_tokens[i], temperature=0, ignore_eos=True, logprobs=0
        )
        for i in range(len(max_new_tokens))
    ]

    kwargs = {"max_num_seqs": max_num_seqs, "use_cb": True} if cb == 1 else {"warmup_shapes": warmup_shapes}
