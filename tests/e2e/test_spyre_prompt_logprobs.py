...
@pytest.mark.skip
def test_prompt_logprobs(backend: str, model: ModelInfo, tp_size: int, monkeypatch: pytest.MonkeyPatch) -> None:
    """
    ...
    for prompt, response in zip(prompts, responses):
        ...
        _compare_prompt_logprobs(expected_logprobs, actual_logprobs, max_different_tokens=1, relative_tolerance=0.15)
...
def _compare_prompt_logprobs(expected: list, actual: list, max_different_tokens: int, relative_tolerance: float):
    ...
    for i in range(len(actual)):
        ...
        for token, actual_logprob in actual_dict.items():
            ...
            assert math.isclose(expected_logprob["logprob"], actual_logprob.logprob, rel_tol=relative_tolerance)
