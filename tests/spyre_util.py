...
def patch_environment(use_cb: bool, warmup_shapes: DecodeWarmupShapes | None, backend: str, monkeypatch):
    # Setup the environment correctly for the LLM

    # ---- For static batching ----
    if warmup_shapes:
        assert not use_cb, "Warmup shapes through environment variables have been deprecated in continuous batching"
...
def patch_warmup_shapes(warmup_shapes: DecodeWarmupShapes | EmbeddingWarmupShapes, monkeypatch):
    ...
    monkeypatch.setenv("VLLM_SPYRE_WARMUP_PROMPT_LENS", ",".join(str(val) for val in warmup_prompt_length))
    monkeypatch.setenv("VLLM_SPYRE_WARMUP_BATCH_SIZES", ",".join(str(val) for val in warmup_batch_size))

    if all(len(s) == 3 for s in warmup_shapes):
        warmup_new_tokens = [t[1] for t in warmup_shapes]
        monkeypatch.setenv("VLLM_SPYRE_WARMUP_NEW_TOKENS", ",".join(str(val) for val in warmup_new_tokens))
...
def get_spyre_model_list(isEmbeddings=False, isScoring=False, full_size_models=False):
    ...
    for model in user_test_model_list.split(","):
        ...
        test_model_list.append(pytest.param(ModelInfo(name=model_path), marks=marks, id=model.strip()))
...
def _default_test_models(isEmbeddings=False, isScoring=False, full_size_models=False):
    ...
    if isEmbeddings:
        model = ModelInfo(
            name="sentence-transformers/all-roberta-large-v1", revision="cf74d8acd4f198de950bf004b262e6accfed5d2c"
        )
        return [pytest.param(model, marks=[pytest.mark.embedding], id=model.name)]

    if isScoring:
        model = ModelInfo(name="cross-encoder/stsb-roberta-large", revision="2b12c2c0088918e76151fd5937b7bba986ef1f98")
    ...
    if not full_size_models:
        ...
        params = [
            ...
            pytest.param(tinygranite_fp8, marks=[pytest.mark.decoder, pytest.mark.quantized], id=tinygranite_fp8.name),
        ]
        return params

    # Full sized decoders
    # The granite 8b fp8 model is not publicly available yet
    granite = ModelInfo(name="ibm-granite/granite-3.3-8b-instruct", revision="51dd4bc2ade4059a6bd87649d68aa11e4fb2529b")
    params = [
...
def create_random_request(
    ...
    tokenizer = AutoTokenizer.from_pretrained(model.name, revision=model.revision)
    if from_model_vocab:
        assert model is not None, "Prompt requested to be generated from model's vocabulary: need to provide model."

        valid_token_ids = sorted([v for v in tokenizer.vocab.values() if v not in tokenizer.all_special_ids])
        ...
        assert len(prompt_token_ids) == num_tokens, f"need {num_tokens} but got {len(prompt_token_ids)}"
