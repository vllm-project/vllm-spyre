    ...
    for prompt_index, (prompt, hf_result, vllm_result) in enumerate(zip(prompts, hf_results, vllm_results)):
        ...
        if len(hf_result["tokens"]) > 0:
            ...
            for i, (hf_token_id, hf_logprob, vllm_token_id, vllm_logprob) in enumerate(
                ...
                logprob_rel_diff = math.fabs(logprob_abs_diff / max(math.fabs(hf_logprob), math.fabs(vllm_logprob)))
        ...
        assert DISABLE_ASSERTS or backend == "sendnn" or hf_result["token_ids"] == vllm_result["token_ids"], (
...
def extract_output(req_output):
    ...
    result["tokens"] = tuple(req_output.outputs[0].logprobs[i][t].decoded_token for i, t in enumerate(token_ids))
    result["logprobs"] = tuple(req_output.outputs[0].logprobs[i][t].logprob for i, t in enumerate(token_ids))

    return result


def generate_cache_for_test_swap_decode_programs_for_cb(model: str | ModelInfo, prompts: list[str], parent_path: str):
    """
