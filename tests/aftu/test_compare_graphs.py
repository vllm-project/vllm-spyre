...
from graph_compare_utils import (collect_graph_files, compare_graphs, 
                                 get_aftu_script_dir, get_aftu_graphs,
                                 get_model_path)
...
@pytest.mark.parametrize(
    "warmup_shape", [(64, 5, 1), (64, 5, 2), (64, 5, 4)])  # (prompt_length/new_tokens/batch_size)
@pytest.mark.parametrize("backend", ["sendnn"])
def test_compare_graphs_static_batching(
    ...
    inference_py_args = [
        ...
        "--compile_dynamic",
        "--min_pad_length", "64",
        "--max_new_tokens", str(warmup_shape[1]),
        "--batch_size", str(warmup_shape[2]),
    ]
