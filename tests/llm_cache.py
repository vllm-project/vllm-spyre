...
class ModelCache(Generic[T]):
    ...
    def set(self, runtime_config: dict, model: T) -> T:
        assert runtime_config not in self._past_runtime_configs, (
            f"Runtime config {runtime_config} was previously cached for type [{self._type()}], error in test ordering!"
        )
...
class LLMCache:
    ...
    def __init__(self):
        self._cache: ModelCache[LLM] = ModelCache[LLM](teardown_method=lambda x: force_engine_shutdown(x))
...
class RemoteOpenAIServerCache:
    ...
    def get_api_server(self, model: str | ModelInfo, server_args: list[str], server_env: dict) -> RemoteOpenAIServer:
...
def get_cached_api_server(model: str, server_args: list[str], server_env: dict) -> RemoteOpenAIServer:
    # Clear other caches first
    LLM_CACHE.clear()
    ENGINE_CACHE.clear()
...
def print_llm_cache_info():
    ...
    print(f"Runtime Server Cache hits: {API_SERVER_CACHE._cache.hits} / misses: {API_SERVER_CACHE._cache.misses}")
    print(f"Engine Core Cache hits: {ENGINE_CACHE._cache.hits} / misses: {ENGINE_CACHE._cache.misses}")
    print("\n-------------------------\n")
