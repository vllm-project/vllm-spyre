...
class GoldenTokenInjector(LogitsProcessor):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device, is_pin_memory: bool):
        ...
        self.tokenizer = get_tokenizer(
            vllm_config.model_config.tokenizer, revision=vllm_config.model_config.revision
        )
    ...
    def update_state(self, batch_update: BatchUpdate | None):
        ...
        for index, params, _, _ in batch_update.added:
            ...
            if params.extra_args and (
                injector_dict := params.extra_args.get("golden_token_injector")
            ):
    ...
    def apply(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        for req_idx, expectation in self.req_states.items():
            ...
            expected_token_id = expectation.token_ids[expectation.current_token_idx]
            ...
            expected_logprob = expectation.logprobs[expectation.current_token_idx]
            ...
            label = (
                f"'{expectation.label}'" if expectation.label is not None else f"idx '{req_idx}'"
            )
            ...
            print(
                f"Golden token injection for request {label}"
                f" at token index '{expectation.current_token_idx}':"
            )
            print(
                f"'{token}' ({old_prob * 100:.2f}%) replaced by"
