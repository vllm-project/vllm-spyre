...
def _xfail_fp8_on_cb_spyre(items):
    """Set an xfail marker on all tests that run quantized models on Continuous
    Batching on Spyre.
    
    """

    xfail_marker = pytest.mark.xfail(
        reason="fp8 is not yet supported on Continuous Batching on Spyre")
    for item in items:
