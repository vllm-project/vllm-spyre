...
Spyre Device Constraints:
    - Minimum batch size: 64 (automatically padded/trimmed)
...
class _SpyreLinear:
    ...
    def _init_spyre_linear(self, layer_prefix: str):
        ...
        logger.warning(
            "%s: no dtype promotion is performed,expect numerical differences to upstream vLLM.",
...
class SpyreMergedColumnParallelLinear(_SpyreLinear, MergedColumnParallelLinear):
    ...
    def forward(self, input_: torch.Tensor):
        output = input_.new_empty(input_.shape[0], self.output_size_per_partition)
...
class SpyreRowParallelLinear(_SpyreLinear, RowParallelLinear):
    ...
    def forward(self, input_):
        output = torch.empty(
            (*input_.shape[:-1], self.output_size_per_partition),
            dtype=input_.dtype,
