...
the version of vLLM that we just upgraded from. See examples [the test matrix here](https://github.com/vllm-project/vllm-spyre/blob/v1.8.0/.github/workflows/test.yml#L83-L173).

### Removing support for a vLLM version

We preiodically raise the vLLM lower bound once all key stakeholders agree it is no longer needed.
...
In order to be ready to run on vllm-spyre, the models must be able to pass tests with [AFTU](https://github.com/foundation-model-stack/aiu-fms-testing-utils) with paged attention and chunked prefill enabled.
