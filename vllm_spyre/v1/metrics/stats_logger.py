...
class FileStatLogger(StatLoggerBase):
    def __init__(self, vllm_config: VllmConfig, engine_index=0):
        ...
        self.perf_file = Path(envs_spyre.VLLM_SPYRE_PERF_METRIC_LOGGING_DIR) / "request_metrics.jsonl"

        if self.enabled and engine_index == 0:
            logger.info("Initializing vllm-spyre perf debug logger. Writing perf info to: %s", str(self.perf_file))
    ...
    def record(
        self, scheduler_stats: SchedulerStats | None, iteration_stats: IterationStats | None, engine_idx: int = 0
    ):
        ...
        text_timestamp = datetime.fromtimestamp(iteration_stats.iteration_timestamp).strftime(self.iso_format)[:-3]
        ...
        for r in iteration_stats.finished_requests:
            ...
            estimated_decode_itl = (r.decode_time - estimated_prefill_interrupt) / max(r.num_generation_tokens - 1, 1)
    ...
    def estimate_prefill_interrupt_lower_bound(self, finished_request: FinishedRequestStats) -> float:
