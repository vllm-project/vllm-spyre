...
class SpyreModelRunner(BaseSpyreModelRunner[SamplingInputBatch,
    ...
    def update_states(self, scheduler_output: SchedulerOutput):
        ...
        for i, req_id in enumerate(req_data.req_ids):
            ...
            # TODO: this should be only called on PP
            # if num_new_tokens == 1:
    ...
    def _prepare_chunked_prefill(self,
        ...
        logger.warning(
            ...
            "Either disable it or report a bug.",
    ...
    def prepare_model_input(
        ...
        if is_prefill:
            ...
            return self._prepare_chunked_prefill(
                scheduler_output)
        else:
...
class ContinuousBatchingSpyreModelRunner(SpyreModelRunner):
    ...
    def _prepare_chunked_prefill(
        ...
        slot_mapping = torch.tensor([request.prefill_slot_mapping[chunk_start:chunk_end]], 
                                    dtype=torch.int64, 
                                    device=self.device) 
