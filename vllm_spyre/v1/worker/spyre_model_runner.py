...
from vllm_spyre.model_executor.model_loader.spyre import BACKEND_LIST, SpyreAttentionMetadata, SpyreCausalLM
...
class BaseSpyreModelRunner(ABC, Generic[InputBatchT, RequestStateT, ModelInputsT]):
    def __init__(
        ...
        if self.model_config is not None:
            ...
            if self.model_config.get_sliding_window():
                logger.warning("Sliding window is not supported on Spyre. The model will run without sliding window.")
    ...
    def _prepare_pad_input_ids(
        ...
        for input_ids_i in input_ids_list:
            ...
            if max_len > seq_len:
                logger.info("Left padding request of length %d tokens to %d tokens.", seq_len, max_len)
            pads = torch.ones(max_len - seq_len, dtype=torch.long, device=input_ids_i.device) * self.pad_token_id
    ...
    def get_kv_cache_spec(self) -> KVCacheSpec:
        ...
        attn_spec = FullAttentionSpec(block_size=block_size, num_kv_heads=1, head_size=1, dtype=torch.float16, **kwargs)
...
class SpyreModelRunner(BaseSpyreModelRunner[SamplingInputBatch, SamplingRequestState, SamplingForwardInputs]):
    ...
    def execute_model(
        ...
        **kwargs,
        ...
        req_ids = scheduler_output.scheduled_new_reqs if is_prefill else self.input_batch.sorted_requests_ids
        sampled_ids = output.sampled_token_ids.tolist()
        for i, req in enumerate(req_ids):
            req_state = self.requests[req.req_id] if not isinstance(req, str) else self.requests[req]
...
class WarmupShapesMixin:
    ...
    def _get_padded_batch_size(self, new_requests: list[NewRequestData]):
        ...
        for request_data in new_requests:
            ...
            new_tokens = request_data.sampling_params.max_tokens if request_data.sampling_params is not None else 0

            updated_spyre_warmup_shapes = [
                shape
                for shape in applicable_spyre_warmup_shapes
                if len(prompt_tokens) <= shape["prompt_length"] and new_tokens <= shape["new_tokens"]
            ]
...
class StaticBatchingSpyreModelRunner(WarmupShapesMixin, SpyreModelRunner):
    ...
    def _prepare_prompt(
        ...
        for request_data in new_requests:
            ...
            input_token_list.append(torch.tensor(prompt_tokens, dtype=torch.long, device=torch.device("cpu")))
        ...
        while len(input_token_list) < padded_batch_size:
            input_token_list.append(torch.zeros(min_pad_length_batch, dtype=torch.long, device=torch.device("cpu")))
...
class ContinuousBatchingSpyreModelRunner(SpyreModelRunner):
    ...
    def get_total_spyre_blocks(self) -> int:
        ...
        logger.info("Maximum concurrency for %s tokens per request: %.2fx", str(max_model_len), max_concurrency)
    ...
    def _prepare_prompt(
        ...
        prompt_token_ids_tensor = torch.tensor(prompt_token_ids, dtype=torch.long, device=torch.device("cpu"))
        ...
        input_tokens, position_ids, mask = self.pad_input_ids(
            [prompt_token_ids_tensor], min_pad_left=left_padding_tkv, min_pad_right=right_padding_tkv
        )
    ...
    def _prepare_decode(
        ...
        left_padded_prompt_mask = torch.tensor(left_padded_prompt_mask, dtype=torch.long, device=self.device)
        block_table = torch.tensor(block_table, dtype=torch.int64)
        slot_mapping = torch.tensor(slot_mapping, dtype=torch.int64)
        self.model.indices = torch.ones(len(cached_request_data.req_ids), dtype=torch.bool, device="cpu")
    ...
    def pad_input_ids(
        ...
        if n_pads_right > 0:
            ...
            logger.info("Right padding request of length %d tokens to %d tokens.", left_pad_len, min_pad_right)
            ...
            mask = torch.nn.functional.pad(mask_left, (0, n_pads_right, 0, n_pads_right), value=-torch.inf)
...
class SpyrePoolingModelRunner(
    WarmupShapesMixin, BaseSpyreModelRunner[PoolingInputBatch, PoolingRequestState, PoolingForwardInputs]
):
    def __init__(
        self,
        vllm_config: VllmConfig,
    ...
    def load_model(self, prompt_lens: Iterable[int], num_decode_tokens: Iterable[int]) -> None:
        ...
        if task == "embed":
            ...
            class_model = AutoModelForSequenceClassification.from_pretrained(self.model_config.model)
            if hasattr(class_model, "bert"):
                ...
                raise ValueError(
                    f"Unsupported model {self.model_config.model}: Expected Bert or Roberta for sequence classification"
                )
        ...
        if envs_spyre.VLLM_SPYRE_DYNAMO_BACKEND in BACKEND_LIST:
            ...
            with utils_spyre.stagger_region(
                ...
                self.model = torch.compile(
                    self.model, mode="default", dynamic=False, backend=envs_spyre.VLLM_SPYRE_DYNAMO_BACKEND
                )
    ...
    def _prepare_prompt(
        ...
        # get position ids and attention mask
        input_tokens, position_ids, mask = self.pad_input_ids(input_token_list, min_pad_length=min_pad_length_batch)
    ...
    def execute_model(
        ...
        **kwargs,
        ...
        pooling_metadata.build_pooling_cursor(num_scheduled_tokens=pooling_metadata.prompt_lens, device=self.device)
        ...
        raw_pooler_output = self.pooler(hidden_states=torch.cat(hidden_state_list), pooling_metadata=pooling_metadata)
