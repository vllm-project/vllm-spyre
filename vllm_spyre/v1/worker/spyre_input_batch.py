...
class SamplingInputBatch(BaseInputBatch[SamplingRequestState]):
    ...
    def __init__(
        ...
        self.logitsprocs_wrappers = [lp for lp in self.logitsprocs.all if isinstance(lp, LogitProcessorWrapper)]
    ...
    def add_request(
        ...
        self.batch_update_builder.added.append((tmp_dense, params, request.prompt_token_ids, request.output_token_ids))

        while tmp_dense > dense_index:
            self.batch_update_builder.moved.append((tmp_dense, tmp_dense - 1, MoveDirectionality.SWAP))
    ...
    def remove_request(self, req_id: str):
        ...
        for tmp_dense in range(dense_index, end_dense_idx):
            self.batch_update_builder.moved.append((tmp_dense, tmp_dense + 1, MoveDirectionality.UNIDIRECTIONAL))
    ...
    def _make_sampling_metadata(self) -> SamplingMetadata:
        ...
        generators = {i: self.generators[idx] for i, idx in enumerate(indices) if self.generators.get(idx) is not None}
