        ...
        if envs.VLLM_TORCH_PROFILER_DIR:
            ...
            if envs_spyre.VLLM_SPYRE_DYNAMO_BACKEND == "sendnn":
                ...
                if envs_spyre.VLLM_SPYRE_AUTOPILOT_OFF:
                    logger.info("VLLM_SPYRE_AUTOPILOT_OFF was set. "
                                "Running with autopilot=0.")

                    # Set autopilot to 0 to disable it
                    options["autopilot"] = "0"

                    # Reconstruct DT_OPT with the updated autopilot value
                    new_dt_opt = ",".join(f"{k}={v}"
                                          for k, v in options.items())
                    os.environ["DT_OPT"] = new_dt_opt
                else:
                    logger.info(
                        "VLLM_SPYRE_AUTOPILOT_OFF wasn't set. Detected "
                        "autopilot=%s", autopilot_opt)
