...
def override(name: str, value: str) -> None:
    if name not in environment_variables:
        raise ValueError(
            f"The variable {name} is not a known \
...
def _backend_backwards_compat() -> str:
    ...
    if val == "sendnn_decoder":
        logger.warning_once("Using 'sendnn_decoder' for VLLM_SPYRE_DYNAMO_BACKEND is deprecated. Use 'sendnn' instead")
        val = "sendnn"
    return val
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    "VLLM_SPYRE_PERF_METRIC_LOGGING_ENABLED": lambda: int(os.getenv("VLLM_SPYRE_PERF_METRIC_LOGGING_ENABLED", 0)),
    # Directory to write performance metric logging files. By default,
    # logs are written to /tmp.
    "VLLM_SPYRE_PERF_METRIC_LOGGING_DIR": lambda: os.getenv("VLLM_SPYRE_PERF_METRIC_LOGGING_DIR", "/tmp"),
    # If set, override the signal handler for vllm-spyre on
    # vLLM V1 + torch_sendnn backend to be able to gracefully
    # shutdown the engine.
    "VLLM_SPYRE_OVERRIDE_SIGNALS_HANDLER": lambda: bool(int(os.getenv("VLLM_SPYRE_OVERRIDE_SIGNALS_HANDLER", "1"))),
    ...
    "VLLM_SPYRE_N_TOKENS_PREFILL_PRIO": lambda: int(os.getenv("VLLM_SPYRE_N_TOKENS_PREFILL_PRIO", "-1")),
    ...
    "VLLM_SPYRE_UPDATE_THREAD_CONFIG": lambda: bool(int(os.getenv("VLLM_SPYRE_UPDATE_THREAD_CONFIG", "1"))),
    ...
    "VLLM_SPYRE_WORKER_LOG_REDIRECT_DIR": lambda: os.getenv("VLLM_SPYRE_WORKER_LOG_REDIRECT_DIR", ""),
    # If set, overrides the default (30 minutes) timeout for
    #  torch.distributed.init_process_group
    "VLLM_SPYRE_GLOO_TIMEOUT_MINUTES": lambda: int(os.getenv("VLLM_SPYRE_GLOO_TIMEOUT_MINUTES", "60")),
    ...
    # are available.
    "VLLM_SPYRE_SIMPLE_COMPILE_BACKEND": lambda: os.getenv("VLLM_SPYRE_SIMPLE_COMPILE_BACKEND", "inductor"),
